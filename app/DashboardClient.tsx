...
export default function DashboardClient() {
  ...
  const totalAtual = latest['Total'] || 748949;
  const fixa = latest['fixa'] || 300406;
  const cript = latest['Cript'] || 17415;
  const invUSA = latest['Investimentos USA'] || 97327;
  const fgts = latest['FGTS'] || 91166;
  ...
  const chart = useMemo(() => {
    const max = Math.max(...series.map((s:any)=>s.total), projectionEnd, 1000000);
    const items = [...series, { date: 'Dez/2027', total: projectionEnd }];
    const aportSeries = items.map((it:any, idx:number)=> ({
      date: it.date,
      total: it.total,
      aport: Math.round((aportes/(items.length-1))*idx)
    }));
    ...
    return {
      map,
      main: buildLine(map.map((m:any)=>({x:m.x, y:m.y1}))),
      sub: buildLine(map.map((m:any)=>({x:m.x, y:m.y2})))
    };
      ...
      <main className="main">
        <section className="hero">
          <div className="heroInner">
            ...
            <div className="topBar" style={{marginTop:16}}>
              <button className="hide" onClick={() => setShow(v => !v)}>
        ...
        <section className="topStats">
          <div className="card stat">
            ...
            <div>
              <div className="kLabel">Patrimônio total</div>
              <div className="kValue">{hideValue(show, money(totalAtual))}</div>
            </div>
          </div>
          <div className="card stat">
            <div className="statIcon">↗</div>
            <div>
              <div className="kLabel">Aporte total</div>
              <div className="kValue">{hideValue(show, money(aportes))}</div>
            </div>
          </div>
          <div className="card stat">
            <div className="statIcon">💲</div>
            <div>
              <div className="kLabel">Rendimentos (juros)</div>
              <div className="kValue">{hideValue(show, money(rendimentos))}</div>
            </div>
          </div>
          <div className="card stat">
            <div className="statIcon">🎯</div>
            <div>
              <div className="kLabel">Projeção Dez/2027</div>
              <div className="kValue">{hideValue(show, money(projectionEnd))}</div>
            </div>
          </div>
        </section>
        ...
        <section className="grid">
          <div className="card">
            ...
            <div className="legend">
              <div><span className="lineChip"></span> Patrimônio projetado</div>
              <div><span className="dotChip"></span> Aportes acumulados</div>
            </div>
            <div className="chart">
              <svg viewBox="0 0 900 290" preserveAspectRatio="none">
                ...
                {chart.map.map((p:any, idx:number)=>(
                  <circle key={idx} cx={p.x} cy={p.y1} r="4" fill="#fff" />
                ))}
              </svg>
            </div>
            <div className="note">
              Regra aplicada na integração: a projeção sempre usa a <strong>última coluna com a data mais atual</strong> da aba <strong>updates</strong>.
              <br />
              Base atual lida da planilha: <strong>{sheet?.lastDate || '07/05/2026'}</strong>.
              <br />
          ...
          <div className="stack">
            <div className="card">
              <div className="marketHead">
                <div className="marketName"><span className="coin btc">₿</span> Bitcoin (BTC)</div>
                <div style={{fontSize:12,color:'#d4d4d4'}}>Variação 24h</div>
              </div>
              <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-end'}}>
                <div className="marketVal">{show ? '$ 62.743,58' : '••••••'}</div>
                <div className="up">+1,28%</div>
              </div>
              <div className="marketMeta">
                <div>
                  <div style={{fontSize:12,color:'#d4d4d4'}}>Tendência</div>
                  <div className="up" style={{fontSize:28}}>ALTA ↑</div>
                </div>
                <div style={{fontSize:12,color:'#ddd',textAlign:'right'}}>Viés positivo<br />curto prazo</div>
              </div>
              <div className="spark">
                <svg viewBox="0 0 220 58" preserveAspectRatio="none">
                  <path d="M0 46 L15 44 L28 38 L39 36 L52 34 L65 29 L79 32 L94 24 L107 27 L122 18 L137 22 L154 14 L171 16 L188 11 L205 15 L220 6" fill="none" stroke="#1fdb73" strokeWidth="3" />
                </svg>
              </div>
              <div className="bullets">
                • Bitcoin segue favorecido por fluxo institucional e ETFs.<br />
                • O principal risco segue sendo juros altos por mais tempo nos EUA.<br />
                • Se continuar acima dos suportes recentes, a tendência é de continuidade positiva.
              </div>
            </div>

            <div className="card">
              <div className="marketHead">
                <div className="marketName"><span className="coin usd">$</span> Dólar (USD/BRL)</div>
                <div style={{fontSize:12,color:'#d4d4d4'}}>Variação 24h</div>
              </div>
              <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-end'}}>
                <div className="marketVal">{show ? 'R$ 5,08' : '••••••'}</div>
                <div className="down">-0,42%</div>
              </div>
              <div className="marketMeta">
                <div>
                  <div style={{fontSize:12,color:'#d4d4d4'}}>Tendência</div>
                  <div className="down" style={{fontSize:28}}>BAIXA ↓</div>
                </div>
                <div style={{fontSize:12,color:'#ddd',textAlign:'right'}}>Viés de alívio<br />curto prazo</div>
              </div>
              <div className="spark">
                <svg viewBox="0 0 220 58" preserveAspectRatio="none">
                  <path d="M0 14 L15 18 L30 20 L45 23 L60 21 L74 25 L90 28 L104 30 L120 34 L136 37 L152 35 L168 41 L184 43 L201 45 L220 49" fill="none" stroke="#ff5264" strokeWidth="3" />
                </svg>
              </div>
              <div className="bullets">
                • O dólar recua quando o mercado enxerga menor pressão inflacionária e maior apetite a risco.<br />
                • Fiscal brasileiro, Selic e FED continuam sendo os motores principais.<br />
                • Em caso de piora global, o câmbio pode inverter rapidamente.
              </div>
            </div>
          </div>
        </section>
        ...
        <section className="bottomGrid">
          <div className="card">
            ...
            <div className="pie">
              <div className="pieCenter">
                ...
                <div style={{fontWeight:900,fontSize:22}}>{hideValue(show, money(24842.71))}</div>
              </div>
            </div>
          ...
          <div className="card">
            <div className="titleRow">
              ...
              <span className="good">▣ Conectado</span>
            </div>
            <table className="table">
              <thead>
                <tr><th>Categoria</th><th>Último valor</th></tr>
              </thead>
              <tbody>
                {Object.entries(latest).filter(([k]) => k !== 'Total').slice(0, 8).map(([k,v], i) => (
                  <tr key={i}><td>{k}</td><td>{show ? money(Number(v)) : '••••••'}</td></tr>
                ))}
              </tbody>
            </table>
            <div className="hint">
              <strong>Leitura da sua planilha:</strong><br />
              • Aba usada: <strong>updates</strong><br />
              • Regra: usa sempre a <strong>última coluna com data mais atual</strong><br />
              • Estrutura esperada nessa aba: primeira coluna com categorias e linha superior com datas.<br />
              • Categorias já reconhecidas no seu modelo: Ações, Saldo Corretora, fundo invest, COE, VGBL, fixa, Cofre, FGTS, Investimentos USA, MOEDAS, Cript e Total.
            </div>
          </div>
            ...
            <div className="forecast">
              {weatherDays.map((d, i) => (
                <div className="day" key={i}>
                  <small>{d[0]}<br />{d[1]}</small>
                  <div style={{fontSize:18,margin:'4px 0'}}>{d[2]}</div>
                  <small>{d[3]} / {d[4]}</small>
                  <em>{d[5]}</em>
                </div>
              ))}
          ...
          <div className="card">
            ...
            <div className="analysis">
              ...
              <ul>
                ...
                <li><strong>Bitcoin:</strong> tende a seguir construtivo no médio prazo, mas com volatilidade bem alta.</li>
                <li><strong>Dólar:</strong> tem espaço para aliviar no curto prazo, mas continua dependente do fiscal brasileiro e do FED.</li>
                <li><strong>Projeção patrimonial:</strong> os aportes mensais seguem sendo o maior motor do crescimento até 2027.</li>
              </ul>
              <div>
                <strong>Destaques da sua última coluna:</strong><br />
